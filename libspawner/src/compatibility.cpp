...
std::string GenerateSpawnerReport(const report_class &rep, const options_class &options, const restrictions_class &restrictions) {
    ...
    osstream << "CreateProcessMethod:       " << (options.login.empty() ? "CreateProcess" : "WithLogon") << std::endl;
    ...
    osstream << "----------------------------------------------" << std::endl;
    osstream << "UserTime:                  " << convert(value_t(unit_time_second, degree_micro), value_t(unit_time_second), static_cast<long double>(rep.processor_time), " (sec)") << std::endl;
    ...
    osstream << "Written:                   " << convert(value_t(unit_memory_byte), value_t(unit_memory_byte, degree_mega), static_cast<long double>(rep.write_transfer_count), " (Mb)") << std::endl;
